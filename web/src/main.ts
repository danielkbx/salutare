...
type GreetingResponse = {
    date_utc: string;
    day_number_utc: number;
    offset: number;
    id: number; // the selected greeting row id (changes with offset)
    greeting: string;
    language: {
        de: string;
        en: string;
 ...
 */
function buildGreetingUrl(): string {
 ...
 */
const BACKGROUND_VARIANTS: string[] = [
    // 0 — Golden Dawn
    `
 ...
 */
function paletteIndexForGreetingId(id: number): number {
 ...
 */
function applyBackground(backgroundCss: string): void {
...
async function loadGreeting(): Promise<void> {
    ...
    if (!res.ok) {
        const apiErr = await parseJsonSafely<ApiError>(res);
        const fallbackText = await res.text().catch(() => "");

        const apiMessage = apiErr?.error;
        const message =
            apiMessage ??
